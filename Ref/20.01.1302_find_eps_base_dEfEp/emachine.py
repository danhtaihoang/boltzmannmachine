...
def generate_seq(n_var,n_seq,n_sample=30,g=1.0):
    ...
    sample_energy = np.einsum('ij,j',ops,w_true)
...
def fit(ops,eps=0.1,max_iter=151,alpha=0.1):
    ...
    for i in range(max_iter):
        ...
        z_data = np.sum(prob)
        prob /= z_data
        ops_expect = np.sum(prob[:,np.newaxis]*ops,axis=0)

        dw = alpha*(ops_expect - w*eps)
        w += dw #alpha*(ops_expect - w*eps)
        
        d_energy = -np.einsum('ij,j',ops,dw)
        #w_iter[i] = w

    return w,dw#,energy,d_energy
#=========================================================================================
